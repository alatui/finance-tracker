class UserStocksController < ApplicationController
  ...
  def create
    if params[:stock_id].present?
      @user_stock = UserStock.new(stock: Stock.find_by(id: params[:stock_id]), user: current_user)
    else
      @user_stock = UserStock.new(stock: Stock.find_or_create(params[:stock_ticker]), user: current_user)
    end

    if @user_stock.stock.nil?
